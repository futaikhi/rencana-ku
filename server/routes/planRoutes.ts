...
function enrichPlanWithProgress(plan: any, userId: string) {
  const planId = plan.id;
  const tasks = queryAll<{ id: string; status: string }>(
    "SELECT id, status FROM tasks WHERE plan_id = ?",
    [planId]
  );
  const milestones = queryAll<{ id: string; status: string }>(
    "SELECT id, status FROM milestones WHERE plan_id = ?",
    [planId]
  );
  const budgetItems = queryAll<{ estimated_amount: number; actual_amount: number }>(
    "SELECT estimated_amount, actual_amount FROM budget_items WHERE plan_id = ?",
    [planId]
  );
  const members = queryAll<{ id: string; user_id: string; role: string; name: string; avatar_url: string }>(
    `SELECT pm.id, pm.user_id, pm.role, u.name, u.avatar_url 
...
router.get("/", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    const myPlans = rawMyPlans.map((p) => enrichPlanWithProgress(p, userId));
    const sharedPlans = rawSharedPlans.map((p) => enrichPlanWithProgress(p, userId));

    // Also get pending invitations for the user
    const pendingInvitations = queryAll(
    ...
    const upcomingMilestones = queryAll(
...
router.post("/", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    // Log activity
    logActivity(planId, userId, "created_plan", `created the plan "${name.trim()}"`, "plan", planId);

    const createdPlan = queryOne("SELECT * FROM plans WHERE id = ?", [planId]);
    const enriched = enrichPlanWithProgress(createdPlan, userId);
...
router.get("/:id", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    const enriched = enrichPlanWithProgress(plan, userId);
...
router.put("/:id", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    logActivity(planId, userId, "updated_plan", `updated the plan details`, "plan", planId);

    const updatedPlan = queryOne("SELECT * FROM plans WHERE id = ?", [planId]);
    const enriched = enrichPlanWithProgress(updatedPlan, userId);
...
router.delete("/:id", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    execute("DELETE FROM plans WHERE id = ?", [planId]);
...
router.post("/:id/leave", authenticateToken, (req: AuthRequest, res: Response) => {
  try {
    ...
    execute("DELETE FROM plan_members WHERE plan_id = ? AND user_id = ?", [planId, userId]);
    logActivity(planId, userId, "left_plan", `${req.user!.name} left the Plan`, "member", userId);
